...
int main(void)
{
    ...
    h_C = (float*)malloc(NGPU * sizeof(float));
    h_partial_C = (float*)malloc(NGPU * sizeof(float));
    if (! h_A || ! h_B || ! h_C|| ! h_partial_C) {
      printf("!!! Not enough memory.\n");
      exit(1);
    }

  // Initialize h_partial_C to zeros
    for (int i = 0; i < NGPU; ++i) {
        h_partial_C[i] = 0;
    }
...
//  cudaSetDevice(cpu_thread_id);
  ...
  if(cpu_thread_id == 0) {
        ...
        cudaMemcpy(h_partial_C + cpu_thread_id, d_C, sizeof(float), cudaMemcpyDeviceToHost);
  ...
  if(cpu_thread_id == 0) {
   ...
   // Aggregate partial dot products from each GPU
    float total_dot_product = 0;
    for (int i = 0; i < NGPU; ++i) {
        total_dot_product += h_partial_C[i];

    }
    *h_C = total_dot_product; 
    // check result
    printf("Check result:\n");
    
    double diff = fabs(h_D - total_dot_product);
