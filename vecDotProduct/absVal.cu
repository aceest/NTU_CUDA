...
// Device code
__global__ void minAbs(const float* A, float* C, int N)
{
    ...
    float temp = FLT_MAX;  // register for each thread
    while (i < N) {
    ...
    while (ib != 0) {
      if(cacheIndex < ib)
        cache[cacheIndex] = fminf(cache[cacheIndex], cache[cacheIndex + ib]);
...
int main(void)
{
    ...
    printf("minimum of the absolute value\n");
    int N;
...
//    printf("The number of blocks per grid:%d\n",blocksPerGrid);
    ...
    minAbs <<< blocksPerGrid, threadsPerBlock, sm >>>(d_A, d_C, N);
